...
<body>
	<div class="fixed">
		<div class="header">
			...
			<div class="datosDelUsuario">
				...
				<c:if test="${not empty plan}">
					<h3 class="puntosUser">Tus tiradas gratis de ultraball
						${usuario.tiradaUltraball}</h3>
					<h3 class="puntosUser">Tus tiradas gratis de masterball
						${usuario.tiradaMasterball}</h3>
				</c:if>
				<h3 class="puntosUser">Tiradas hasta el proximo Epico
					${tiradas}</h3>
			</div>
		...
		<c:if test="${sessionScope.principiante}">
			<div class="container">
				<div class="centrar">
					<img class="nestball" id="nestball" alt="pokemon"
						src="images/nestball.png">
				</div>
			</div>
		</c:if>
		...
		<div class="centrar">
			<div class="datosDelUsuario">
				<c:if test="${not empty error}">
					<h3 class="puntosUser">${error}</h3>
					<br>
				</c:if>
				<c:if test="${not empty errorPokemonedas}">
					<h3 class="puntosUser">${errorPokemonedas}</h3>
					<br>
				</c:if>
				<c:if test="${not empty comprado}">
					<h3 class="puntosUser">${comprado}</h3>
					<br>
				</c:if>
		...
		<button type="button" class="btn btn-primary margenBoton"
			data-toggle="modal" data-target="#myModal">Probabilidades</button>
		<div class="modal fade" id="myModal" tabindex="-1" role="dialog"
			aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="myModalLabel">Probabilidades del
							Gachapon</h5>
		...
		<button type="button" class="btn btn-primary margenBoton"
			data-toggle="modal" data-target="#myModal2">Tienda Pokemon</button>
								...
								<form method="POST" class="objetos" action="gachapon-comprar">
									<div
										...
										<div class="d-flex flex-column align-items-center"
											style="width: 33%;">
											<label class="vBuscado" style="text-transform: uppercase;">
												${pokemon.nombre} </label> <br> <img alt="${pokemon.nombre}"
												class="img-fluid mb-2" style="max-height: 70px;"
												src="images/sprites/${pokemon.nombre}/${pokemon.imagenFrente}">
											<br>
											<div>
												Rareza: <span class="precio">${pokemon.rareza} </span>
											</div>
											<div>
												<input type="hidden" name="idPokemon" value="${pokemon.id}" />
												<input class="btn btn-warning btn-lg d-block" type="submit"
													value="Comprar"></input>
											</div>
											<br>
										</div>
