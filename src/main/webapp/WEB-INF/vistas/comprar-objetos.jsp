			...
			<div class="d-inline-flex flex-column mb-2">
				<c:forEach items="${listaUsuarioObjetos}" var="usuarioObjeto">
					<div class="form-control">
						...
						<div>
							Precio: <span class="precio-objeto${usuarioObjeto.objeto.id}">${usuarioObjeto.objeto.precio}
								puntos</span>
						</div>
						<input type="number" class="form-control col-3 puntos-gastados"
							id="objeto${usuarioObjeto.objeto.id}" name="cantidad" value="0"
							min="0" max="99" /> <input type="hidden" name="idsObjetos"
							value="${usuarioObjeto.objeto.id}" /> <br>
					</div>
				</c:forEach>
			</div>
