			...
			<tbody>
				<c:forEach var="plan" items="${planes}" varStatus="status">
					<tr>
						...
						<td><c:choose>
								<c:when
									test="${nombreUsuarioPlan == 'Basico' && plan.nombre == 'Basico'}">
									Plan activo
								</c:when>
								<c:when
									test="${nombreUsuarioPlan == 'Basico' && plan.nombre == 'Premium'}">
									<a class="btn btn-primary" href="mejorar-plan/${plan.id}"
										role="button">Mejorar</a>
								</c:when>
								<c:when
									test="${nombreUsuarioPlan == 'Premium' && plan.nombre == 'Premium'}">
									Plan activo
								</c:when>
								<c:when test="${empty nombreUsuarioPlan}">
